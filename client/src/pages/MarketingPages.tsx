...
export function ContactPage() {
  ...
  return <MarketingLayout><PageHero eyebrow="Contact / Book a Demo" title={<>Bring us the challenge. We will help you find the <em>next move.</em></>} intro="Tell us where your AI workforce needs to do more. We will use the conversation to understand the context, discuss the opportunity, and decide whether agents DX is a useful fit." aside={<><p className="aside-label">What to expect</p><h3>A focused conversation about your <em>operating reality.</em></h3><p>Share the context. We will help make the next step more concrete.</p></>} /><section className="contact-section"><div className="shell contact-section__grid"><div className="contact-form-wrap"><p className="eyebrow"><span /> Start the conversation</p><h2>Tell us what needs to <em>move forward.</em></h2><form onSubmit={submit} noValidate>{[["name", "Name", "Your name", "text"], ["email", "Work email", "you@company.com", "email"], ["company", "Company", "Your company", "text"]].map(([key, label, placeholder, type]) => <label key={key}>{label}<input type={type} name={key} placeholder={placeholder} value={form[key as keyof ContactFormState]} onChange={event => setForm({ ...form, [key]: event.target.value })} /></label>)}<label>Message<textarea name="message" placeholder="What would a stronger AI workforce make possible for your team?" rows={5} value={form.message} onChange={event => setForm({ ...form, message: event.target.value })} /></label>{error && <p className="form-message form-message--error" role="alert">{error}</p>}{submitted && <p className="form-message" role="status">Your demo request is ready. Open the booking calendar to choose a time.</p>}<button className="form-submit" type="submit">Prepare Demo Request <ArrowRight size={18} /></button></form></div><aside className="calendar-placeholder"><div className="calendar-placeholder__top"><span>agents DX</span><span className="calendar-placeholder__dot" /></div><div className="calendar-placeholder__body"><div className="calendar-icon"><div /><span /></div><p className="eyebrow"><span /> Live booking</p><h3>Find a time<br />that works.</h3><p>Book a demo with our experts to see how your AI workforce can engage and convert.</p><a href="https://cal.com/agentsdx/platform" data-cal-link="agentsdx/platform" data-cal-namespace="platform" data-cal-config='{"layout":"month_view","useSlotsViewOnSmallScreen":"true"}' className="button-link button-link--primary">Book Demo <ArrowRight size={16} /></a><div className="calendar-slots"><span /><span /><span /></div></div></aside></div></section></MarketingLayout>;
}
