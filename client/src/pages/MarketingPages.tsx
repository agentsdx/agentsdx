...
export function LegalPage({ title }: { title: "Privacy Policy" | "Terms of Service" }) {
  ...
  return <MarketingLayout><section className="legal-hero"><div className="shell legal-hero__grid"><div><p className="eyebrow eyebrow--light"><span /> {document.eyebrow}</p><h1>{title}</h1><p>{document.summary}</p></div><aside><p>Working legal draft</p><strong>Last updated<br />20 August 2026</strong><span>Attorney review required before publication or reliance.</span></aside></div></section><section className="legal-page"><div className="shell legal-page__grid"><aside className="legal-page__nav"><p>In this document</p><nav aria-label={`${title} sections`}>{document.sections.map(section => <a key={section.id} href={`#${section.id}`}>{section.title.replace(/^\d+\.\s/, "")}</a>)}</nav><a href="mailto:cs@AgentsDX.com">Questions? Contact us <ArrowRight size={15} /></a></aside><article className="legal-page__document"><div className="legal-page__notice"><ShieldCheck size={27} /><div><h2>{document.noticeTitle}</h2><p>{document.notice}</p></div></div><p className="legal-page__lead">{document.lead}</p>{document.sections.map(section => <section id={section.id} key={section.id}><h2>{section.title}</h2>{section.paragraphs.map(paragraph => <p key={paragraph}>{paragraph}</p>)}{section.bullets && <ul>{section.bullets.map(item => <li key={item}>{item}</li>)}</ul>}</section>)}</article></div></section></MarketingLayout>;
}
