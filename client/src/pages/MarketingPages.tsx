...
const connectedChannels = [
  { number: "01", icon: MessageCircle, title: "WhatsApp", text: "Keep immediate customer conversations connected to the same workforce context and next-step logic." },
  { number: "02", icon: CircleDot, title: "Instagram", text: "Turn direct-message interest into a considered path rather than an isolated social interaction." },
  { number: "03", icon: MessageSquareText, title: "Messenger", text: "Give customer conversations a helpful response rhythm that stays aligned with your operating standards." },
  { number: "04", icon: Zap, title: "Telegram", text: "Route fast-moving customer signals into the workforce view without losing the context behind them." },
  { number: "05", icon: Mail, title: "Email", text: "Carry the conversation forward with a clearer record of what has already been asked, answered, and agreed." },
  { number: "06", icon: MessageCircle, title: "Web chat", text: "Turn the first on-site question into a useful, context-aware path instead of a dead end or a delayed reply." },
  { number: "07", icon: Target, title: "SMS", text: "Keep concise customer updates tied to the same workforce context, response standards, and next move." },
];
...
export function FeaturesPage() {
  return (
    <MarketingLayout>
      ...
      <section className="features-channels-section"><div className="shell features-channels-section__grid"><div className="features-channels-canvas" aria-label="Illustrative connected customer channels"><div className="features-channels-canvas__top"><span>agents DX / CHANNEL NETWORK</span><strong>7 CHANNELS</strong></div><div className="features-channels-canvas__core"><span>DX</span><small>WORKFORCE</small></div>{connectedChannels.map(channel => { const Icon = channel.icon; return <div className={`features-channels-canvas__node features-channels-canvas__node--${channel.number}`} key={channel.title}><Icon size={17} /><span>{channel.title.toUpperCase()}</span></div>; })}<div className="features-channels-canvas__footer"><i /> One workforce. Seven channels. Shared context.</div></div><div><p className="eyebrow"><span /> Connected channels</p><h2>Meet customers where they are. <em>Run one workforce across every channel.</em></h2><p>agents DX runs across WhatsApp, Instagram, Messenger, Telegram, Email, Web chat, and SMS. Every channel begins with the same shared context, controls, and next-step logic.</p><div className="features-channels-list">{connectedChannels.map(channel => { const Icon = channel.icon; return <article key={channel.title}><span>{channel.number}</span><Icon size={20} strokeWidth={1.8} /><div><h3>{channel.title}</h3><p>{channel.text}</p></div></article>; })}</div></div></div></section>
...
const legalDocuments: Record<"privacy" | "terms", { eyebrow: string; summary: string; lead: string; noticeTitle: string; notice: string; sections: LegalSection[] }> = {
  privacy: {
    eyebrow: "Privacy & data",
    summary: "How agents DX collects, uses, safeguards, and gives people choices around information connected to the website and platform.",
    lead: "This Privacy Policy is a working description of how agents DX may process personal information when you visit our website, contact us, book a demonstration, use our platform, or otherwise interact with our services.",
    noticeTitle: "Working draft — legal review required",
    notice: "This page should be reviewed and completed by qualified counsel before publication. It requires the final legal entity name, registered address, jurisdictional requirements, processor list, retention schedule, and any applicable regulatory disclosures.",
    sections: [
      { id: "information", title: "1. Information we may collect", paragraphs: ["The information we process depends on how you interact with agents DX. We seek to collect information that is relevant to providing, securing, supporting, and improving the service."], bullets: ["Contact and account information, such as name, business email address, company name, role, and account credentials.", "Workspace and service information, such as configuration details, authorised knowledge sources, conversation context, and records created through use of the platform.", "Communications and support information, including messages, booking requests, feedback, and correspondence with our team.", "Technical and usage information, such as device, browser, IP address, diagnostic logs, page interactions, and service-performance data.", "Commercial information necessary to administer a subscription, invoice, or other agreed business relationship."] },
      { id: "use", title: "2. How we may use information", paragraphs: ["agents DX may use information to operate the website and platform, deliver requested services, maintain security, respond to enquiries, communicate about the relationship, and improve product reliability and usability."], bullets: ["Provide, personalise, administer, and support the agents DX service.", "Respond to demonstrations, support requests, and other communications.", "Protect accounts, investigate misuse, and maintain the security and integrity of our systems.", "Measure service performance, troubleshoot defects, and improve product features.", "Meet legal, accounting, contractual, or compliance obligations that apply to our business relationship."] },
      { id: "basis", title: "3. Legal bases and consent", paragraphs: ["Where applicable law requires a legal basis for processing, agents DX will rely on the basis appropriate to the context, which may include performance of a contract, legitimate interests, compliance with legal obligations, or consent. We will seek consent where required and provide a way to withdraw it where withdrawal is available."] },
      { id: "sharing", title: "4. How information may be shared", paragraphs: ["We do not sell personal information in exchange for money. We may share information only where it is reasonably necessary for the purposes described in this policy or where the law requires it."], bullets: ["With service providers and professional advisers that help us operate, secure, support, or assess the service under appropriate confidentiality and data-protection terms.", "With authorised users, administrators, or other parties within the relevant customer workspace, according to the permissions they control.", "With competent authorities, courts, or regulators where disclosure is legally required or necessary to protect rights, safety, or security.", "In connection with a proposed or completed corporate transaction, subject to appropriate safeguards and notice where required.", "With another party when you direct us to do so or give permission for a specific disclosure."] },
      { id: "retention", title: "5. Retention and deletion", paragraphs: ["We retain information only for as long as reasonably necessary to provide the service, meet the purpose for which it was collected, resolve disputes, enforce agreements, or satisfy legal and recordkeeping obligations. Final retention periods must be confirmed in the published policy and any customer agreement."] },
      { id: "security", title: "6. Security", paragraphs: ["We use administrative, technical, and organisational measures designed to protect information against unauthorised access, loss, misuse, alteration, or disclosure. No method of transmission or storage is completely secure, so users should also safeguard their credentials and report suspected account compromise promptly."] },
      { id: "transfers", title: "7. International transfers", paragraphs: ["If information is transferred across borders, agents DX will use the safeguards required by applicable law. The final published policy should identify the relevant transfer mechanisms and destinations where disclosure is required."] },
      { id: "choices", title: "8. Your choices and rights", paragraphs: ["Depending on your location and applicable law, you may have rights to request access to, correction of, deletion of, restriction of, or portability for certain personal information, and to object to certain processing. We may need to verify a request and may retain information where an exception or legal obligation applies."] },
      { id: "cookies", title: "9. Cookies and similar technologies", paragraphs: ["The website and service may use cookies, local storage, and similar technologies to keep sessions working, understand performance, remember preferences, and improve the experience. The final published policy should be paired with any required cookie notice or consent mechanism for the jurisdictions in which agents DX operates."] },
      { id: "children", title: "10. Children’s information", paragraphs: ["agents DX is intended for business users and is not directed to children. We do not knowingly collect personal information from children in circumstances where consent from a parent or guardian would be required by law."] },
      { id: "updates", title: "11. Changes to this policy", paragraphs: ["We may update this policy as the service, law, or our information practices change. The final published version should display an effective date, and material changes should be communicated in the manner required by applicable law or contract."] },
      { id: "contact", title: "12. Privacy contact", paragraphs: ["For privacy-related questions or requests, contact cs@AgentsDX.com. The final policy should also identify the responsible legal entity, postal address, and any required data-protection contact or representative."] },
    ],
  },
  terms: {
    eyebrow: "Service terms",
    summary: "The working commercial framework for accessing and using the agents DX website, platform, communications, and related services.",
    lead: "These Terms of Service are a working draft for the relationship between agents DX and the person or organisation that accesses the website, creates an account, uses the platform, or otherwise receives the service.",
    noticeTitle: "Working draft — legal review required",
    notice: "This page must be reviewed and completed by qualified counsel before publication. It requires the final contracting entity, jurisdiction, payment terms, service commitments, limitation amounts, dispute process, and customer-agreement hierarchy.",
    sections: [
      { id: "acceptance", title: "1. Acceptance and scope", paragraphs: ["By accessing or using agents DX, you agree to these Terms and to any order form, subscription agreement, acceptable-use policy, or other written terms that apply to your relationship with us. If you use the service on behalf of an organisation, you represent that you have authority to bind that organisation."] },
      { id: "accounts", title: "2. Accounts and access", paragraphs: ["You are responsible for providing accurate account information, maintaining the confidentiality of credentials, and promptly notifying us of suspected unauthorised access. You must ensure that users you invite to a workspace use the service in accordance with these Terms and your internal authorisation controls."] },
      { id: "subscriptions", title: "3. Subscriptions, fees, and renewals", paragraphs: ["Paid services, billing intervals, renewals, taxes, refunds, and payment obligations will be governed by the applicable order flow or written commercial agreement. Unless a written agreement states otherwise, fees are due in the currency, cadence, and manner shown at the time of purchase. Final payment and renewal language must be approved for the jurisdictions in which agents DX contracts."] },
      { id: "acceptable-use", title: "4. Acceptable use", paragraphs: ["You may use agents DX only for lawful business purposes and in accordance with these Terms. You must not use the service in a way that harms people, compromises systems, violates rights, or bypasses agreed access controls."], bullets: ["Do not use the service to break the law, infringe intellectual-property or privacy rights, or facilitate fraud, abuse, harassment, or deception.", "Do not probe, disrupt, reverse engineer, overload, or attempt to gain unauthorised access to the platform, accounts, or connected services.", "Do not upload or instruct the system to process information unless you have the necessary rights, notices, permissions, and lawful basis to do so.", "Do not represent generated or automated content as human-originated where disclosure is required by law, contract, or applicable platform rules.", "Do not use the service in a way that creates prohibited high-risk outcomes without appropriate human oversight and safeguards."] },
      { id: "data-ai", title: "5. Customer data and AI-assisted output", paragraphs: ["You retain responsibility for the information, instructions, knowledge sources, and other content you submit to the service. You grant agents DX only the rights needed to host, process, and support that content in order to provide the service. You are responsible for reviewing AI-assisted output before relying on it and for configuring appropriate human oversight, permissions, and safeguards for your use case."] },
      { id: "ip", title: "6. Intellectual property", paragraphs: ["agents DX and its licensors retain all rights in the service, software, interfaces, documentation, brand assets, and related materials, except for rights expressly granted to you in writing. Subject to these Terms and payment of applicable fees, agents DX grants you a limited, non-exclusive, non-transferable, revocable right to access and use the service for your internal business purposes during the applicable subscription term."] },
      { id: "third-parties", title: "7. Third-party services and integrations", paragraphs: ["The service may connect with third-party services. Your use of those services is governed by the third party’s terms and privacy practices. agents DX is not responsible for third-party products, availability, content, or security practices, except to the extent a written agreement expressly states otherwise."] },
      { id: "availability", title: "8. Availability, support, and changes", paragraphs: ["We may update, modify, suspend, or discontinue features to maintain security, improve the service, comply with law, or evolve the product. Any service-level commitments, support response times, maintenance procedures, or notice obligations will be set out in the applicable written agreement, if any."] },
      { id: "confidentiality", title: "9. Confidentiality", paragraphs: ["Each party may receive confidential information from the other. Each party will use the other party’s confidential information only to perform or receive the service, protect it using reasonable care, and disclose it only to people who need to know it and are bound by appropriate confidentiality obligations, unless disclosure is required by law."] },
      { id: "disclaimers", title: "10. Disclaimers", paragraphs: ["Except where a written agreement expressly provides otherwise and to the extent permitted by applicable law, the service is provided on an “as is” and “as available” basis. agents DX does not guarantee that the service will be uninterrupted, error-free, suitable for every purpose, or that AI-assisted output will be complete, accurate, or appropriate for a particular decision without human review."] },
      { id: "liability", title: "11. Limitation of liability", paragraphs: ["The final published terms must state the legally reviewed scope of any exclusions, caps, and exceptions to liability. Nothing in these working terms is intended to exclude liability that cannot lawfully be excluded or limited. Any agreed liability cap must be confirmed in the applicable customer agreement and governing jurisdiction."] },
      { id: "indemnity", title: "12. Indemnity", paragraphs: ["The final published terms should set out any mutually agreed indemnity obligations, including procedures for notice, defence, cooperation, and settlement. Those obligations must be reviewed in light of the service model, applicable law, and the final customer contract."] },
      { id: "termination", title: "13. Suspension and termination", paragraphs: ["We may suspend or terminate access where reasonably necessary to address non-payment, security risks, legal requirements, material breach, or harmful use. You may stop using the service at any time, subject to any written subscription commitment. The final terms should address data export, post-termination access, and deletion periods."] },
      { id: "law", title: "14. Governing law and disputes", paragraphs: ["The governing law, courts or arbitration forum, venue, and any required pre-dispute process have not yet been provided. These must be supplied by authorised agents DX counsel before publication and before the terms are relied upon."] },
      { id: "changes", title: "15. Changes to these terms", paragraphs: ["We may update these Terms as the service, law, or commercial model changes. The final published version should identify the effective date and the notice process for material changes, including how a customer may object or discontinue use where required."] },
      { id: "contact", title: "16. Contact", paragraphs: ["For questions about these Terms, contact cs@AgentsDX.com. The final published version should also identify the legal contracting entity and its registered address."] },
    ],
  },
};

export function LegalPage({ title }: { title: "Privacy Policy" | "Terms of Service" }) {
  const documentKey = title === "Privacy Policy" ? "privacy" : "terms";
  const document = legalDocuments[documentKey];
  const completionItems = documentKey === "terms"
    ? ["Contracting entity", "Jurisdiction", "Payment & service terms", "Liability & dispute process"]
    : ["Legal entity & address", "Jurisdictional disclosures", "Processor & retention records", "Regulatory contact details"];
  return <MarketingLayout><section className="legal-hero"><div className="shell legal-hero__grid"><div><p className="eyebrow eyebrow--light"><span /> {document.eyebrow}</p><h1>{title}</h1><p>{document.summary}</p><div className="legal-hero__meta"><span>WORKING DOCUMENT</span><i /> <span>LAST REVIEWED 20 AUG 2026</span></div></div><aside className="legal-hero__review"><p><ShieldCheck size={15} /> Counsel completion</p><h2>{document.noticeTitle}</h2><strong>{document.notice}</strong><ul>{completionItems.map(item => <li key={item}>{item}</li>)}</ul></aside></div></section><section className="legal-page"><div className="shell legal-page__grid"><aside className="legal-page__nav"><p>In this document</p><nav aria-label={`${title} sections`}>{document.sections.map(section => <a key={section.id} href={`#${section.id}`}>{section.title.replace(/^\d+\.\s/, "")}</a>)}</nav><a href="mailto:cs@AgentsDX.com">Questions? Contact us <ArrowRight size={15} /></a></aside><article className="legal-page__document"><header className="legal-page__document-header"><div><p>Document overview</p><h2>{title}</h2></div><span>Working draft<br />Attorney review required</span></header><div className="legal-page__notice"><ShieldCheck size={27} /><div><h2>{document.noticeTitle}</h2><p>{document.notice}</p></div></div><p className="legal-page__lead">{document.lead}</p>{document.sections.map(section => <section id={section.id} key={section.id}><h2>{section.title}</h2>{section.paragraphs.map(paragraph => <p key={paragraph}>{paragraph}</p>)}{section.bullets && <ul>{section.bullets.map(item => <li key={item}>{item}</li>)}</ul>}</section>)}</article></div></section></MarketingLayout>;
}
